...
<body>
	<div class="jumbotron">
		...
		<table class="table table-dark table-hover text-center">
			<thead>
				<tr>
					<th>순번</th><th>이름</th><th>코드</th>
					<th>코드</th><th>코드</th><th>주소</th>
				</tr>
			</thead>
			<tbody>
				<% for (SPRO_DaeGu sd : list) { %>
				<tr>
					<td><%=sd.getIdx()%></td><td><%=sd.getName()%></td><td><%=sd.getCode1_name()%></td>
					<td><%=sd.getCode2_name()%></td><td><%=sd.getCode4_name()%></td><td><%=sd.getDoro_addr()%></td>
				</tr>
				<% } %>
			</tbody>
		</table>
